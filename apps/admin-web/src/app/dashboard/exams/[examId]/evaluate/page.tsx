  ...
  if (!exam?.can_evaluate_sheets && exam) {
    return (
      <div className="card" style={{ padding: 24 }}>
        ...
        <Link href="/dashboard/exams" className="btn btn-ghost" style={{ marginTop: 12 }}>← Back to exams</Link>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="card bento-glass" style={{ marginBottom: 24, padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>Evaluate answer sheets</h1>
          <div style={{ fontSize: 13, color: "var(--text-muted)" }}>{exam?.title || "Exam"} · Vision OCR + async grading</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <Link href="/dashboard/exams/corrections" className="btn btn-ghost" style={btn}>Past corrections</Link>
          <button className="btn btn-ghost" style={btn} onClick={() => router.push("/dashboard/exams")}>← Exams</button>
        </div>
      </div>
