...
export default function AdmissionsPage() {
  ...
  return (
    ...
    >
      ...
      <div className="gw-pipeline" role="tablist" aria-label="Filter by stage">
        <button
          type="button"
          role="tab"
          aria-selected={stageFilter === null}
          className={`gw-pipeline-stage${stageFilter === null ? " gw-pipeline-stage-active" : ""}`}
          onClick={showAllCandidates}
        >
          <span className="gw-pipeline-count">{totalCandidates}</span>
          <span className="gw-pipeline-label">Total Enquiries</span>
        </button>
        {STAGES.map((stage) => {
          ...
          return (
            ...
            >
              <span className="gw-pipeline-count">{pipeline[stage] ?? 0}</span>
              <span className="gw-pipeline-label">{stageLabel(stage)}</span>
            </button>
          );
        })}
      </div>
