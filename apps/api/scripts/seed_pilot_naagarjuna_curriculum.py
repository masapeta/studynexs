"""Gate 2 (G2-12): SSC Class 10 Maths CurriculumPack for Naagarjuna pilot tenant.

Uses reconciled Batch 1 architecture: draft pack → chapters/topics/LOs → approve_pack()
(KG spine + eager RAG + audit). Idempotent: skips if approved pack already exists.
...
async def main() -> None:
    async with async_session_factory() as db:
        ...
        if not school:
            print(f"School not found (tenant_slug='{TENANT}'). Run seed_pilot_naagarjuna.py first.")
            return
        ...
        if not cls:
            print("Class 10 not found for pilot school.")
            return
        ...
        if not maths:
            print("Mathematics subject (MAT10) not found.")
            return
        ...
        if not ay:
            print("Academic year not found for Class 10.")
            return
        ...
        pack = await svc.create_pack(
            ...
            PackCreate(
                ...
                publisher="Placeholder — update when school confirms textbook",
                edition="2026-27 pilot",
        ...
        await db.commit()
        print(f"Seeded approved CurriculumPack for {TENANT} Class 10 Maths")
        print(
        ...
        print("  Replace chapters when Naagarjuna sends their official syllabus.")
