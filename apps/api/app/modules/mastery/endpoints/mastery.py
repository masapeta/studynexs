...
async def _build_learning_evidence_chain(
    db: AsyncSession,
    *,
    school_id: uuid.UUID,
    tenant_slug: str,
    ...
    approved_evaluation_ids = sorted(
        {eid for row in chain_exams for eid in row.approved_evaluation_ids}, key=str
    )
...
async def flag_evidence_chain(
    ...
    chain = await _build_learning_evidence_chain(
        db,
        school_id=school_id,
